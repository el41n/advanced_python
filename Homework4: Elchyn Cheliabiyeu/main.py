...
if __name__ == '__main__':
    ...
    for sync in syncronize:
        sync.acquire()
    p1 = multiprocessing.Process(name="Process 1", target=print_num, args=(0, 2, syncronize))
    p2 = multiprocessing.Process(name="Process 2", target=print_num, args=(1, 2, syncronize))
    p1.start()
    p2.start()
    syncronize[0].release()
    p1.join()
